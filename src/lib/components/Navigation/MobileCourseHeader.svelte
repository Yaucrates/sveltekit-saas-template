<script lang="ts">
	...
	import { House, LayoutList, BookOpen, ClipboardList, Award, LayoutDashboard, Settings, CircleQuestionMark } from '@lucide/svelte';
...
<header class="sm:hidden sticky top-0 z-50 flex items-center justify-between bg-white border-b border-stone-100 px-5 h-14">
	<a href="/courses/{courseSlug}/" onclick={close}>
		<ImpartNameLogo />
	</a>
...
>
	<div class="flex-1 flex flex-col gap-3 text-sm">
		...
		<a
			href="/dashboard"
			onclick={close}
			class="flex items-center gap-2 p-2 rounded-sm transition-colors duration-200 hover:bg-stone-100 text-stone-700"
		>
