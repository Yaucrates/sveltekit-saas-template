...
import { introToCSDetails } from './introduction-to-computer-science';
...
export type CourseItem =
	| (CourseChapter & { kind: 'chapter' })
	| (CourseAssignment & { kind: 'assignment' });
...
] as Omit<Course, 'id' | 'slug' | 'sections' | 'chapters' | 'assignments'>[]).map((course, id) => ({
    id,
    slug: toLink(course.title),
    sections: [] as CourseSection[],
    chapters: [] as CourseChapter[],
    assignments: [] as CourseAssignment[],
    ...course
}));

export function getDetailedCourse(slug: string): Course | null {
	const course = courses.find(c => c.slug === slug);
	if (!course || (course.chapters.length === 0 && course.assignments.length === 0)) return null;
	return course;
}

export function getCourseItems(course: Course): CourseItem[] {
	const chapters: CourseItem[] = course.chapters.map(c => ({ ...c, kind: 'chapter' as const }));
	const assignments: CourseItem[] = course.assignments.map(a => ({ ...a, kind: 'assignment' as const }));
	return [...chapters, ...assignments].sort((a, b) => a.section - b.section || a.order - b.order);
}
